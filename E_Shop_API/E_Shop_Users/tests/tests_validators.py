...
class BirthdayValidatorTest(TestCase):
    ...
    def test_incorrect_date_format(self):
        ...
        with self.assertRaises(ValueError, msg=UserErrorMessages.INCORRECT_DATE_FORMAT.value):
            birthday_validator(datetime.strptime(self.incorrect_date, "%Y-%m-%d").date())

    def test_future_date(self):
        """ Checking the future date format"""
        with self.assertRaises(ValidationError, msg=UserErrorMessages.FUTURE_DATE.value):
    ...
    def test_less_1930(self):
        ...
        with self.assertRaises(ValidationError, msg=UserErrorMessages.DATE_GREATE_1930.value):
...
class PasswordValidatorTest(TestCase):
    ...
    def test_short_password(self):
        ...
        with self.assertRaises(ValidationError, msg=UserErrorMessages.SHORT_PASSWORD.value):
            validate_password('short')

    def test_password_without_numbers(self):
        """  Checking if the password without numbers"""
        with self.assertRaises(ValidationError, msg=UserErrorMessages.PASSWORD_WITHOUT_NUMBERS.value):
    ...
    def test_password_without_capital_letters(self):
        ...
        with self.assertRaises(ValidationError, msg=UserErrorMessages.PASSWORD_WITHOUT_CAPITAL_LETTERS.value):
